    ...
    with torch.no_grad():
        for batch in loader:
            ...
            for i, model in enumerate(models):
                batch.x = x_data.to(device)

                # Make sure this model is last!
                if "20230323-102724" in str(model_paths[i]):
                    batch.x = batch.x[:, :10]  # Drop scattering flag
                    # Undo new scaling
                    batch.x[:, 3] /= 4
                    batch.x[:, 3] += 0.06
                    # Add old scaling
                ...
                azi_out_cos += weights[i] * torch.cos(pred_azi)
                azi_out_sin += weights[i] * torch.sin(pred_azi)
                zen_out += weights[i] * pred_zen

            azi_out = torch.atan2(azi_out_sin, azi_out_cos)
            
            pred_angles = torch.stack([azi_out, zen_out], dim=1)
            predictions.append(pred_angles.cpu())
...
def make_predictions(
    ...
    model_paths = [item for sublist in mpaths for item in sublist]
    ...
    sub = {
        ...
        "azimuth": model_output[:, 0],
        "zenith": model_output[:, 1],
    }
...
if __name__ == "__main__":
    ...
    weights = [0.16738237, 0.10536909, 0.37259442, 0.35226272]
    # weights = None
